...
NSString * const OOSUICKeyChainStoreErrorDomain = @"cn.ctyun.uickeychainstore";
static NSString *_defaultService;

@interface OOSUICKeyChainStore ()

@end

@implementation OOSUICKeyChainStore
...
+ (OOSUICKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (OOSUICKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:OOSUICKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = OOSUICKeyChainStoreItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:OOSUICKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(OOSUICKeyChainStoreProtocolType)protocolType authenticationType:(OOSUICKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = OOSUICKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = OOSUICKeyChainStoreAccessibilityAfterFirstUnlock;
}
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    OOSUICKeyChainStore *keychain = [OOSUICKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
#pragma clang diagnostic pop
    ...
    if (status == errSecSuccess || status == errSecInteractionNotAllowed) {
        ...
        if (unexpectedError) {
            OOSDDLogError(@"error: [%@] %@", @(unexpectedError.code), NSLocalizedString(@"Unexpected error has occurred.", nil));
            if (error) {
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:(OOSUICKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == OOSUICKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == OOSUICKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(OOSUICKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == OOSUICKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (void)setSynchronizable:(BOOL)synchronizable
{
    ...
    if (_authenticationPolicy) {
        OOSDDLogDebug(@"%@", @"Cannot specify both an authenticationPolicy and a synchronizable");
    }
}
    ...
    SecRequestSharedWebCredential((__bridge CFStringRef)domain, (__bridge CFStringRef)account, ^(CFArrayRef credentials, CFErrorRef error) {
        if (error) {
            ...
            if (e.code != errSecItemNotFound) {
                OOSDDLogError(@"error: [%@] %@", @(e.code), e.localizedDescription);
            }
        }
...
- (NSMutableDictionary *)query
{
    ...
    if (_authenticationPrompt) {
        if (floor(NSFoundationVersionNumber) > floor(1047.25)) { // iOS 8+ (NSFoundationVersionNumber_iOS_7_1)
            ...
            OOSDDLogDebug(@"%@", @"Unavailable 'authenticationPrompt' attribute on iOS versions prior to 8.0.");
        }
    }
#endif
...
#if TARGET_OS_IOS
            OOSDDLogDebug(@"%@", @"Unavailable 'Touch ID integration' on iOS versions prior to 8.0.");
#else
            OOSDDLogDebug(@"%@", @"Unavailable 'Touch ID integration' on OS X versions prior to 10.10.");
#endif
        }
    } else {
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == OOSUICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            OOSDDLogDebug(@"%@", @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            OOSDDLogDebug(@"%@", @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case OOSUICKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case OOSUICKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case OOSUICKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case OOSUICKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case OOSUICKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case OOSUICKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case OOSUICKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case OOSUICKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case OOSUICKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case OOSUICKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case OOSUICKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case OOSUICKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case OOSUICKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case OOSUICKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case OOSUICKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case OOSUICKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case OOSUICKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case OOSUICKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case OOSUICKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case OOSUICKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case OOSUICKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case OOSUICKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case OOSUICKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case OOSUICKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case OOSUICKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case OOSUICKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case OOSUICKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case OOSUICKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case OOSUICKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case OOSUICKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case OOSUICKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case OOSUICKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case OOSUICKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case OOSUICKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case OOSUICKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case OOSUICKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case OOSUICKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case OOSUICKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case OOSUICKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case OOSUICKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case OOSUICKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case OOSUICKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case OOSUICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case OOSUICKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case OOSUICKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case OOSUICKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:OOSUICKeyChainStoreErrorDomain code:OOSUICKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
    OOSDDLogError(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}

+ (NSError *)conversionError:(NSString *)message
{
    NSError *error = [NSError errorWithDomain:OOSUICKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    OOSDDLogError(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:OOSUICKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
    OOSDDLogDebug(@"OSStatus error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}

+ (NSError *)unexpectedError:(NSString *)message
{
    NSError *error = [NSError errorWithDomain:OOSUICKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    OOSDDLogError(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
