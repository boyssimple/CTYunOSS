...
OOSmtl_propertyAttributes *OOSmtl_copyPropertyAttributes (objc_property_t property) {
    ...
    OOSmtl_propertyAttributes *attributes = calloc(1, sizeof(OOSmtl_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate OOSmtl_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = OOSmtl_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = OOSmtl_propertyMemoryManagementPolicyRetain;
            break;
