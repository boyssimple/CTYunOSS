...
#import "OOSEXTRuntimeExtensions.h"
#import "OOSEXTScope.h"
#import "OOSCocoaLumberjack.h"

void OOSmtl_loadMTLNSCoding(){
}

// Used in archives to store the modelVersion of the archived instance.
static NSString * const OOSMTLModelVersionKey = @"OOSMTLModelVersion";

// Used to cache the reflection performed in +allowedSecureCodingClassesByPropertyKey.
static void *OOSMTLModelCachedAllowedClassesKey = &OOSMTLModelCachedAllowedClassesKey;
...
static NSSet *encodablePropertyKeysForClass(Class modelClass) {
	return [[modelClass encodingBehaviorsByPropertyKey] keysOfEntriesPassingTest:^ BOOL (NSString *propertyKey, NSNumber *behavior, BOOL *stop) {
		return behavior.unsignedIntegerValue != OOSMTLModelEncodingBehaviorExcluded;
	}];
}
...
@implementation OOSMTLModel (NSCoding)
...
+ (NSDictionary *)encodingBehaviorsByPropertyKey {
	...
	for (NSString *key in propertyKeys) {
		...
		OOSmtl_propertyAttributes *attributes = OOSmtl_copyPropertyAttributes(property);
		@onExit {
			free(attributes);
		};

		OOSMTLModelEncodingBehavior behavior = (attributes->weak ? OOSMTLModelEncodingBehaviorConditional : OOSMTLModelEncodingBehaviorUnconditional);
...
+ (NSDictionary *)allowedSecureCodingClassesByPropertyKey {
	NSDictionary *cachedClasses = objc_getAssociatedObject(self, OOSMTLModelCachedAllowedClassesKey);
	...
	objc_setAssociatedObject(self, OOSMTLModelCachedAllowedClassesKey, allowedClasses, OBJC_ASSOCIATION_COPY);
...
- (id)decodeValueForKey:(NSString *)key withCoder:(NSCoder *)coder modelVersion:(NSUInteger)modelVersion {
	...
	SEL selector = OOSMTLSelectorWithCapitalizedKeyPattern("decode", key, "WithCoder:modelVersion:");
	...
	@try {
		...
		OOSDDLogError(@"*** Caught exception decoding value for key \"%@\" on class %@: %@", key, self.class, ex);
		@throw ex;
	}
}
...
- (instancetype)initWithCoder:(NSCoder *)coder {
	...
	if (requiresSecureCoding) {
		version = [coder decodeObjectOfClass:NSNumber.class forKey:OOSMTLModelVersionKey];
	} else {
		version = [coder decodeObjectForKey:OOSMTLModelVersionKey];
	}
	
	if (version.unsignedIntegerValue > self.class.modelVersion) {
	...
	if (self == nil) OOSDDLogError(@"*** Could not unarchive %@: %@", self.class, error);
...
- (void)encodeWithCoder:(NSCoder *)coder {
	...
	[coder encodeObject:@(self.class.modelVersion) forKey:OOSMTLModelVersionKey];
	...
	[self.dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
		@try {
			...
			switch ([encodingBehaviors[key] unsignedIntegerValue]) {
				...
				case OOSMTLModelEncodingBehaviorExcluded:
					break;
					
				case OOSMTLModelEncodingBehaviorUnconditional:
					[coder encodeObject:value forKey:key];
					break;
					
				case OOSMTLModelEncodingBehaviorConditional:
			...
			OOSDDLogError(@"*** Caught exception encoding value for key \"%@\" on class %@: %@", key, self.class, ex);
			@throw ex;
		}
	}];
}
