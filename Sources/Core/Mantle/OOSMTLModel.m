...
static BOOL MTLValidateAndSetValue(id obj, NSString *key, id value, BOOL forceUpdate, NSError **error) {
	...
	@try {
		...
		OOSDDLogError(@"*** Caught exception setting key \"%@\" : %@", key, ex);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
	...
	while (!stop && ![cls isEqual:OOSMTLModel.class]) {
		unsigned count = 0;
...
- (void)mergeValueForKey:(NSString *)key fromModel:(OOSMTLModel *)model {
	...
	SEL selector = OOSMTLSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
...
- (BOOL)isEqual:(OOSMTLModel *)model {
