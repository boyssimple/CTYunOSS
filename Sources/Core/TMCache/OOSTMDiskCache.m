#import "OOSTMDiskCache.h"
#import "OOSTMCacheBackgroundTaskManager.h"

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
#import <UIKit/UIKit.h>
#endif

#define OOSTMDiskCacheError(error) if (error) { NSLog(@"%@ (%d) ERROR: %@", \
                                    [[NSString stringWithUTF8String:__FILE__] lastPathComponent], \
                                    __LINE__, [error localizedDescription]); }

static id <OOSTMCacheBackgroundTaskManager> OOSTMCacheBackgroundTaskManager;

NSString * const OOSTMDiskCachePrefix = @"cn.ctyun.TMDiskCache";
NSString * const OOSTMDiskCacheSharedName = @"TMDiskCacheShared";

@interface OOSTMDiskCache ()
...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath
{
    ...
    if (self = [super init]) {
        _name = [name copy];
        _queue = [OOSTMDiskCache sharedQueue];
        ...
        NSString *pathComponent = [[NSString alloc] initWithFormat:@"%@.%@", OOSTMDiskCachePrefix, _name];
        _cacheURL = [NSURL fileURLWithPathComponents:@[ rootPath, pathComponent ]];

        __weak OOSTMDiskCache *weakSelf = self;

        dispatch_async(_queue, ^{
            OOSTMDiskCache *strongSelf = weakSelf;
...
{
    return [[NSString alloc] initWithFormat:@"%@.%@.%p", OOSTMDiskCachePrefix, _name, self];
}
...
+ (instancetype)sharedCache
{
    ...
    dispatch_once(&predicate, ^{
        cache = [[self alloc] initWithName:OOSTMDiskCacheSharedName];
    });
...
+ (dispatch_queue_t)sharedQueue
{
    ...
    dispatch_once(&predicate, ^{
        queue = dispatch_queue_create([OOSTMDiskCachePrefix UTF8String], DISPATCH_QUEUE_SERIAL);
    });
...
+ (dispatch_queue_t)sharedTrashQueue
{
    ...
    dispatch_once(&predicate, ^{
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.trash", OOSTMDiskCachePrefix];
...
+ (NSURL *)sharedTrashURL
{
    ...
    dispatch_once(&predicate, ^{
        sharedTrashURL = [[[NSURL alloc] initFileURLWithPath:NSTemporaryDirectory()] URLByAppendingPathComponent:OOSTMDiskCachePrefix isDirectory:YES];
...
+(BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSURL *uniqueTrashURL = [[OOSTMDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString];
...
+ (void)emptyTrash
{
    UIBackgroundTaskIdentifier taskID = [OOSTMCacheBackgroundTaskManager beginBackgroundTask];
    ...
    dispatch_async([self sharedTrashQueue], ^{        
        ...
        [OOSTMCacheBackgroundTaskManager endBackgroundTask:taskID];
    });
}
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    BOOL trashed = [OOSTMDiskCache moveItemAtURLToTrash:fileURL];
    if (!trashed)
        return NO;
    
    [OOSTMDiskCache emptyTrash];
...
- (void)objectForKey:(NSString *)key block:(OOSTMDiskCacheObjectBlock)block
{
...
- (void)fileURLForKey:(NSString *)key block:(OOSTMDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(OOSTMDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(OOSTMDiskCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToSize:(NSUInteger)trimByteCount block:(OOSTMDiskCacheBlock)block
{
    if (trimByteCount == 0) {
...
- (void)trimToDate:(NSDate *)trimDate block:(OOSTMDiskCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToSizeByDate:(NSUInteger)trimByteCount block:(OOSTMDiskCacheBlock)block
{
    if (trimByteCount == 0) {
        [self removeAllObjects:block];
        return;
...
- (void)removeAllObjects:(OOSTMDiskCacheBlock)block
{
    UIBackgroundTaskIdentifier taskID = [OOSTMCacheBackgroundTaskManager beginBackgroundTask];
    ...
    dispatch_async(_queue, ^{
        ...
        [OOSTMDiskCache moveItemAtURLToTrash:strongSelf->_cacheURL];
        [OOSTMDiskCache emptyTrash];
...
- (void)enumerateObjectsWithBlock:(OOSTMDiskCacheObjectBlock)block completionBlock:(OOSTMDiskCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id <NSCoding>)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(OOSTMDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        objectForKey = object;
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(OOSTMDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key
{
    ...
    [self setObject:object forKey:key block:^(OOSTMDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(OOSTMDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSize:(NSUInteger)byteCount
{
    ...
    [self trimToSize:byteCount block:^(OOSTMDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(OOSTMDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSizeByDate:(NSUInteger)byteCount
{
    ...
    [self trimToSizeByDate:byteCount block:^(OOSTMDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(OOSTMDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(OOSTMDiskCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(OOSTMDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (OOSTMDiskCacheObjectBlock)willAddObjectBlock
{
    __block OOSTMDiskCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(OOSTMDiskCacheObjectBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
- (OOSTMDiskCacheObjectBlock)willRemoveObjectBlock
{
    __block OOSTMDiskCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(OOSTMDiskCacheObjectBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
- (OOSTMDiskCacheBlock)willRemoveAllObjectsBlock
{
    __block OOSTMDiskCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(OOSTMDiskCacheBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
- (OOSTMDiskCacheObjectBlock)didAddObjectBlock
{
    __block OOSTMDiskCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(OOSTMDiskCacheObjectBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
- (OOSTMDiskCacheObjectBlock)didRemoveObjectBlock
{
    __block OOSTMDiskCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(OOSTMDiskCacheObjectBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
- (OOSTMDiskCacheBlock)didRemoveAllObjectsBlock
{
    __block OOSTMDiskCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(OOSTMDiskCacheBlock)block
{
    __weak OOSTMDiskCache *weakSelf = self;
...
+ (void)setBackgroundTaskManager:(id <OOSTMCacheBackgroundTaskManager>)backgroundTaskManager {
    OOSTMCacheBackgroundTaskManager = backgroundTaskManager;
}
