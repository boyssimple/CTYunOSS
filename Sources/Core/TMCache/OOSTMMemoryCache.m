#import "OOSTMMemoryCache.h"

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
#import <UIKit/UIKit.h>
#endif

NSString * const OOSTMMemoryCachePrefix = @"cn.ctyun.TMMemoryCache";

@interface OOSTMMemoryCache ()
...
@implementation OOSTMMemoryCache
...
{
    if (self = [super init]) {
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.%p", OOSTMMemoryCachePrefix, self];
...
- (void)handleMemoryWarning
{
    ...
    __weak OOSTMMemoryCache *weakSelf = self;
    
    dispatch_async(_queue, ^{
        OOSTMMemoryCache *strongSelf = weakSelf;
        if (!strongSelf)
            return;
...
- (void)trimToAgeLimitRecursively
{
    ...
    dispatch_after(time, _queue, ^(void){
        ...
        __weak OOSTMMemoryCache *weakSelf = strongSelf;
...
- (void)objectForKey:(NSString *)key block:(OOSTMMemoryCacheObjectBlock)block
{
...
- (void)setObject:(id)object forKey:(NSString *)key block:(OOSTMMemoryCacheObjectBlock)block
{
    [self setObject:object forKey:key withCost:0 block:block];
}

- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost block:(OOSTMMemoryCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(OOSTMMemoryCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToDate:(NSDate *)trimDate block:(OOSTMMemoryCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToCost:(NSUInteger)cost block:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (void)trimToCostByDate:(NSUInteger)cost block:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (void)removeAllObjects:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (void)enumerateObjectsWithBlock:(OOSTMMemoryCacheObjectBlock)block completionBlock:(OOSTMMemoryCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(OOSTMMemoryCache *cache, NSString *key, id object) {
        objectForKey = object;
...
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost
{
    ...
    [self setObject:object forKey:key withCost:cost block:^(OOSTMMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(OOSTMMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(OOSTMMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCost:(NSUInteger)cost
{
    ...
    [self trimToCost:cost block:^(OOSTMMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCostByDate:(NSUInteger)cost
{
    ...
    [self trimToCostByDate:cost block:^(OOSTMMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(OOSTMMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(OOSTMMemoryCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(OOSTMMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (OOSTMMemoryCacheObjectBlock)willAddObjectBlock
{
    __block OOSTMMemoryCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(OOSTMMemoryCacheObjectBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheObjectBlock)willRemoveObjectBlock
{
    __block OOSTMMemoryCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(OOSTMMemoryCacheObjectBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheBlock)willRemoveAllObjectsBlock
{
    __block OOSTMMemoryCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheObjectBlock)didAddObjectBlock
{
    __block OOSTMMemoryCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(OOSTMMemoryCacheObjectBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheObjectBlock)didRemoveObjectBlock
{
    __block OOSTMMemoryCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(OOSTMMemoryCacheObjectBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheBlock)didRemoveAllObjectsBlock
{
    __block OOSTMMemoryCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheBlock)didReceiveMemoryWarningBlock
{
    __block OOSTMMemoryCacheBlock block = nil;
...
- (void)setDidReceiveMemoryWarningBlock:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
...
- (OOSTMMemoryCacheBlock)didEnterBackgroundBlock
{
    __block OOSTMMemoryCacheBlock block = nil;
...
- (void)setDidEnterBackgroundBlock:(OOSTMMemoryCacheBlock)block
{
    __weak OOSTMMemoryCache *weakSelf = self;
