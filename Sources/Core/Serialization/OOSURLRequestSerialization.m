...
#import "OOSValidation.h"
#import "OOSSerialization.h"
#import "OOSCategory.h"
#import "OOSClientContext.h"

@interface NSMutableURLRequest (OOSRequestSerializer)

- (void)OOS_validateHTTPMethodAndBody;

@end

@implementation NSMutableURLRequest (OOSRequestSerializer)
...
@interface OOSJSONRequestSerializer()
...
@implementation OOSJSONRequestSerializer
...
- (OOSTask *)serializeRequest:(NSMutableURLRequest *)request
    ...
    if ([parameters objectForKey:@"clientContext"]) {
       ...
       forHTTPHeaderField:OOSClientContextHeader];
        [request setValue:@"base64"
       forHTTPHeaderField:OOSClientContextHeaderEncoding];
    ...
    OOSJSONDictionary *inputRules = [[OOSJSONDictionary alloc] initWithDictionary:[actionRules objectForKey:@"input"] JSONDefinitionRule:shapeRules];
    ...
    [OOSXMLRequestSerializer constructURIandHeadersAndBody:request
                                                     rules:inputRules
    ...
    if (!request.HTTPBodyStream) {
        NSData *bodyData = [OOSJSONBuilder jsonDataForDictionary:parameters actionName:self.actionName serviceDefinitionRule:self.serviceDefinitionJSON error:&error];
        if (!error) {
            if (headers[@"Content-Encoding"] && [headers[@"Content-Encoding"] rangeOfString:@"gzip"].location != NSNotFound) {
                //gzip the body
//                request.HTTPBody = [bodyData OOSgzip_gzippedData];
            } else {
    ...
    [request OOS_validateHTTPMethodAndBody];
...
@interface OOSXMLRequestSerializer()
...
@implementation OOSXMLRequestSerializer
...
- (OOSTask *)serializeRequest:(NSMutableURLRequest *)request
    ...
    NSString *ruleURIStr = [actionHTTPRule objectForKey:@"requestUri"];
    NSDictionary *shapeRules = [self.serviceDefinitionJSON objectForKey:@"shapes"];
    OOSJSONDictionary *inputRules = [[OOSJSONDictionary alloc] initWithDictionary:[anActionRules objectForKey:@"input"] JSONDefinitionRule:shapeRules];
    ...
    if (!error) {
        //construct HTTPBody only if HTTPBodyStream is nil
        if (!request.HTTPBodyStream && request.HTTPBody == nil) {
            request.HTTPBody = [OOSXMLBuilder xmlDataForDictionary:parameters
            ...
            if ([rulesType isEqualToString:@"integer"] || [rulesType isEqualToString:@"long"] || [rulesType isEqualToString:@"float"] || [rulesType isEqualToString:@"double"]) {
                ...
                if ([value isKindOfClass:[NSNumber class]]) {
                    ...
                    if ([memberRules[@"location"] isEqualToString:@"header"]) {
                        ...
                        valueStr = [timeStampDate OOS_stringValue:OOSDateRFC822DateFormat1];
                    } else {
                        valueStr = [value stringValue];
                    }
            ...
            if ([memberRules[@"location"] isEqualToString:@"uri"]) {
                ...
                if ([rawURI rangeOfString:keyToFind].location != NSNotFound) {
                    rawURI = [rawURI stringByReplacingOccurrencesOfString:keyToFind
                                                               withString:[valueStr OOS_stringWithURLEncoding]];
                    ...
                    rawURI = [rawURI stringByReplacingOccurrencesOfString:greedyKeyToFind
                                                               withString:[valueStr OOS_stringWithURLEncodingPathWithoutPriorDecoding]];
                }
            ...
            if (([xmlElementName isEqualToString:@"Body"] || [xmlElementName isEqualToString:@"body"]) && [memberRules[@"streaming"] boolValue]) {
                if ([value isKindOfClass:[NSURL class]]) {
                    if ([value checkResourceIsReachableAndReturnError:&blockErr]) {
						// TODO: 由于不支持stream类型，将文件转换成NSData类型
//                        request.HTTPBodyStream = [NSInputStream inputStreamWithURL:value];
						request.HTTPBody = [NSData dataWithContentsOfURL:value];
                    } else {
...
//                        request.HTTPBodyStream = [NSInputStream inputStreamWithData:value];
            ...
            if([memberRules[@"shape"] isEqualToString:@"BlobStream"]){
                OOSDDLogDebug(@"value type = %@", [value class]);
    ...
    if (!isValid) {
        if (error) {
            *error = blockErr;
            if (*error == nil) {
                *error = [NSError errorWithDomain:OOSValidationErrorDomain code:OOSValidationUnknownError userInfo:[NSDictionary dictionaryWithObject:@"Unknown error happened while enumerating rules" forKey:NSLocalizedDescriptionKey]];
            }
        }
        return NO;
    }
    ...
    if ([queryStringDictionary count]) {
        ...
        for (NSString *key in sortedKeys) {
            if ([queryString length] == 0 && uriSchemaContainsQuestionMark == NO) {
                queryString = [NSString stringWithFormat:@"?%@=%@",[key OOS_stringWithURLEncoding],[queryStringDictionary[key] OOS_stringWithURLEncoding]];
            } else {
                NSString *appendString = [NSString stringWithFormat:@"&%@=%@",[key OOS_stringWithURLEncoding],[queryStringDictionary[key] OOS_stringWithURLEncoding]];
    ...
    if (r.location != NSNotFound) {
        if (error) {
            *error = [NSError errorWithDomain:OOSValidationErrorDomain code:OOSValidationURIIsInvalid userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"the constructed request queryString is invalid:%@",rawURI] forKey:NSLocalizedDescriptionKey]];
        }
        ...
        if (!request.URL) {
            if (error) {
                *error = [NSError errorWithDomain:OOSValidationErrorDomain code:OOSValidationURIIsInvalid userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"unable the assigned URL to request, URL may be invalid:%@",finalURL] forKey:NSLocalizedDescriptionKey]];
            }
            return NO;
        }
...
@interface OOSQueryStringRequestSerializer()
...
@implementation OOSQueryStringRequestSerializer
...
- (instancetype)initWithJSONDefinition:(NSDictionary *)JSONDefinition
    ...
    if (self = [super init]) {
        _serviceDefinitionJSON = JSONDefinition;
        if (_serviceDefinitionJSON == nil) {
            OOSDDLogError(@"serviceDefinitionJSON of is nil.");
            return nil;
        }
        _actionName = actionName;
    }
    ...
    for (NSString *key in parameters) {
        ...
        if ([obj isKindOfClass:[NSDictionary class]]) {
            ...
            if ([obj isKindOfClass:[NSString class]]) {
                [queryString appendString:[key OOS_stringWithURLEncoding]];
                [queryString appendString:@"="];
                NSString *urlEncodedString = [obj OOS_stringWithURLEncoding];
                [queryString appendString:urlEncodedString];
            } else if ([obj isKindOfClass:[NSNumber class]]) {
                [queryString appendString:[key OOS_stringWithURLEncoding]];
                [queryString appendString:@"="];
                [queryString appendString:[[obj stringValue] OOS_stringWithURLEncoding]];
            } else {
                OOSDDLogError(@"key[%@] is invalid.", key);
                [queryString appendString:[key OOS_stringWithURLEncoding]];
                [queryString appendString:@"="];
                [queryString appendString:[[obj description]OOS_stringWithURLEncoding]];
            }
        }
    }
...
- (OOSTask *)serializeRequest:(NSMutableURLRequest *)request
    ...
    NSError *error = nil;
    NSDictionary *formattedParams = [OOSQueryParamBuilder buildFormattedParams:parameters
                                                                    actionName:self.actionName
