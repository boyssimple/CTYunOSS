...
@interface OOSJSONResponseSerializer()
...
@implementation OOSJSONResponseSerializer
...
- (id)responseObjectForResponse:(NSHTTPURLResponse *)response
	...
	if([OOSDDLog sharedInstance].logLevel & OOSDDLogFlagDebug){
		if ([data isKindOfClass:[NSData class]]) {
			if ([data length] <= 100 * 1024) {
				OOSDDLogDebug(@"Response body:\n%@", [[NSString alloc] initWithData:data
																		   encoding:NSUTF8StringEncoding]);
			} else {
				OOSDDLogDebug(@"Response body (Partial data. The first 100KB is displayed.):\n%@", [[NSString alloc] initWithData:[data subdataWithRange:NSMakeRange(0, 100 * 1024)]
																														 encoding:NSUTF8StringEncoding]);
	...
	if (responseContentTypeStr) {
		if ([responseContentTypeStr rangeOfString:@"text/html"].location != NSNotFound) {
			...
			if (error) {
				...
				*error = [NSError errorWithDomain:OOSServiceErrorDomain
											 code:OOSServiceErrorUnknown
	...
	//parse JSON data
	result = [OOSJSONParser dictionaryForJsonData:data response:response actionName:self.actionName serviceDefinitionRule:self.serviceDefinitionJSON error:error];
	
	//Parse OOSServiceError
	if ([result isKindOfClass:[NSDictionary class]]) {
		...
		OOSJSONDictionary *outputRules = [[OOSJSONDictionary alloc] initWithDictionary:[anActionRules objectForKey:@"output"] JSONDefinitionRule:shapeRules];
		result = [OOSXMLResponseSerializer parseResponse:response rules:outputRules bodyDictionary:[result mutableCopy] error:error];
		
		NSNumber *errorCode = [[CoreService errorCodeDictionary] objectForKey:[[[result objectForKey:@"__type"] componentsSeparatedByString:@"#"] lastObject]];
		if (errorCode != nil) {
			if (error) {
				*error = [NSError errorWithDomain:OOSServiceErrorDomain
...
@interface OOSXMLResponseSerializer()
...
@implementation OOSXMLResponseSerializer
...
- (BOOL)validateResponse:(NSHTTPURLResponse *)response
			 fromRequest:(NSURLRequest *)request
					data:(id)data
				   error:(NSError *__autoreleasing *)error {
	//Validation already performed during XML Parsing in OOSXMLParser Class.
	return YES;
}
...
- (id)responseObjectForResponse:(NSHTTPURLResponse *)response
	...
	// http://docs.OOS.amazon.com/AmazonS3/latest/API/RESTObjectCOPY.html
	...
	if ([resultDic count] == 0) {
		...
		resultDic = [[OOSXMLParser sharedInstance] dictionaryForXMLData:data
	...
	resultDic = [OOSXMLResponseSerializer parseResponse:response rules:outputRules bodyDictionary:resultDic error:error];
	
	//Parse OOSServiceError
	NSDictionary *errorInfo = resultDic[@"Error"];
	if (errorInfo) {
		if (errorInfo[@"Code"] && [CoreService errorCodeDictionary][errorInfo[@"Code"]]) {
			if (error && (*error == nil)) {
				*error = [NSError errorWithDomain:OOSServiceErrorDomain
											 code:[[CoreService errorCodeDictionary][errorInfo[@"Code"]] integerValue]
										 userInfo:errorInfo
						  ];
			}
		}
